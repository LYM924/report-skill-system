...
基于本地工单明细数据（`data/技术支持工单明细.xlsx`）和 Confluence 上现有周报模板，生成标准化的技术支持周报。工单问题分析、TOP问题分析、高频问题分析等均从技术支持工单明细.xlsx 的工单摘要字段提取数据。
...
**一步并行拉数据 → 审计检查 → 直接输出报告 → [可选] 发布到 Confluence，中间不需要确认。**

1. 运行 `weekly_data.py --json` 获取结构化数据（自动计算周次和日期范围）
2. **【强制】运行 `weekly_data.py --audit` 审计检查**，确认全部检查项通过
3. 模板来源：`output/SKILL.md`（完整报告模板，含所有章节）
4. 业务指标数据：`weekly_data.py` 自动从 Confluence 获取（pageId: 286888251）
5. 上周报告参考：仅用于简述描述参考和格式对齐
6. **输出报告时逐项对照审计结果**，确保每个数字与源数据一致
7. **【可选】运行 `weekly_data.py --publish`** 将报告发布到 Confluence（26年技术支持周报 子页面，覆盖同周次文档）
8. 不需要探索 Agent、不需要分步确认
...
- 从本地 Excel（`data/技术支持工单明细.xlsx`）读取工单明细和超期工单
- 按报告周期筛选本周数据
...
输出路径：`projects/report-system/output/周报/{周次}-技术支持周报.md`（所有报表统一保存在此固定目录下，不新建日期文件夹）
...
- **翡翠看板**: 翡翠周报数据看板 (pageId: 278883515)
- **业务指标**: 翡翠技术支持业务指标数据 (pageId: 286888251)
- **故障明细**: 2026年-故障&工单&安全数据 (pageId: 257088525)
- **空间**: FCSY
