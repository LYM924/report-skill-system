...
2. **本地 Excel 文件**（`data/技术支持工单明细.xlsx`）
...
```bash
python3 projects/report-system/weekly-report/weekly_data.py --json
```
...
**文件位置**: `data/技术支持工单明细.xlsx`（`projects/report-system/data/`）
...
wb = load_workbook('data/技术支持工单明细.xlsx', data_only=True)
ws = wb.active
...
wb = load_workbook('projects/report-system/data/技术支持工单明细.xlsx', data_only=True)
ws = wb.active
