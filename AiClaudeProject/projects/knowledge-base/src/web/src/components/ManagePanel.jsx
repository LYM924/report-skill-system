...
function ManagePanel({ isDark }) {
  ...
  const [publishDeptIds, setPublishDeptIds] = useState([]);
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      {/* 编辑 FAQ */}
      {activeSection === 'faq' && <FaqEditor isDark={isDark} />}

      {/* 上传文档 section */}
      {activeSection === 'upload' && (
        <Card style={{ borderRadius: 12, marginTop: 20, border: '1px solid #e2e8f0' }}>
          <Empty description="请点击上方「上传文档」卡片选择文件" />
        </Card>
      )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <div style={{ flex: 1 }}>
              ...
              <DeptCascader
                value={publishDeptIds}
                onChange={(ids, names) => { setPublishDeptIds(ids); setPublishDept(names.join(', ')); }}
                placeholder="选择部门"
                style={{ marginTop: 4 }}
              />
            </div>
            <div style={{ flex: 1 }}>
              ...
              <ModuleSelect value={publishModule} onChange={setPublishModule} placeholder="输入模块名搜索..." style={{ marginTop: 4 }} />
            </div>
          </div>
          <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <div style={{ flex: 1 }}>
              ...
              <DeptCascader
                value={uploadDeptIds}
                onChange={(ids, names) => { setUploadDeptIds(ids); setUploadDept(names.join(', ')); }}
                placeholder="选择部门"
                style={{ marginTop: 4 }}
              />
            </div>
